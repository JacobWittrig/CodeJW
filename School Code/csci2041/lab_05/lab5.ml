...
let rec scale factor stream =
  makeStream (first stream) stream (fun this state -> (((first (rest state)) * factor), rest state));;
 

let sum left right =
  makeStream ((first left) + (first right)) (left, right) (fun this state -> (((first (rest (fst state))) + (first (rest (snd state)))), ((rest (fst state)),(rest (snd state)))));;
