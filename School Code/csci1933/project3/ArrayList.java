public class ArrayList<T extends Comparable<T>> implements List<T> {
  ...
  //method to check if the list is sorted for more complex methods such as reverse
...
//checks to see if it is still sorted
    return true;
  ...
  public int indexOf(T element){
    ...
    if(isSorted){
      int location=0;
      int addSize=this.size/2;
      while(addSize>5){
        if(a[location+addSize].compareTo(element)<0){
          location+=addSize;
        }
        addSize/=2;
      }
